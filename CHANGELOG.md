...
- 重构 `_conf_schema.json`，按插件更新、AstrBot 框架更新、通知、网络和调试分组，减少配置页面的顶层项目数量。
- 新增框架定时检查的“自动更新”开关；关闭后只检查并通知，不下载、更新或重启。
- 旧版扁平配置会自动迁移到新的嵌套配置组，旧键保留为隐藏兼容字段。

## v2.7.3

- 新增 `plugin_auto_update` 配置项，默认开启以保持原有行为。关闭后，定时任务仅检查插件更新并向管理员通知，不会更新插件或触发重启。

## v2.7.2

- 新增 `更新管理帮助`（`updatemanagerhelp`），按插件更新、插件维护和 AstrBot 框架分类返回全部管理员指令。
- `测试插件更新日志` 更名为 `测试插件管理日志`（保留旧命令兼容），直接返回最多 5 个本机插件的模拟汇报和最近 CHANGELOG 小节；框架更新开启时附加最近一次 AstrBot 发布日志。
- 合并转发插件或 AstrBot 更新日志时不再截断内容；仅在平台不支持合并转发、降级为普通文本时保留总长度限制。
- 新增 `update_report_fields` 配置项，可自定义“发现 N 个插件需要更新”通知中显示插件名称、插件 ID、版本变化、仓库链接和作者；字段顺序固定。
- 插件市场与自定义 GitHub 更新源均会提供本次命中的仓库链接；未提供的元数据统一显示为“未知”，不使用其他字段补充。
- 插件更新日志通知的每个条目现在固定以 `【插件名称】` 标题开头，不受更新汇报字段配置影响。
