# ⬆️ AstrBot 插件更新管理器 astrbot_plugin_update_manager(v2.3.0)
...
- **适配新版 AstrBot 插件市场**：
  - 插件市场 API 已迁移至 `cloud.astrbot.app`，数据键名从"插件名"变为"作者/插件名"，并新增 `$meta` 元数据键，导致旧版本无法匹配到任何在线插件。本版本已完全适配新格式，同时兼容旧格式数据源作为兜底。
  - **版本号机制适配**：新版市场版本号可能带有 `v` 前缀（来自发布标签），比较前会自动归一化，避免误判。
  - **更新方式与官方一致**：优先使用市场提供的 `download_url`（市场托管的指定版本安装包）进行更新，确保更新到的版本与市场展示一致；旧版 AstrBot 核心不支持该参数时自动回退为仓库拉取。
  - **匹配逻辑与官方 WebUI 对齐**：优先按仓库地址精确匹配，其次按插件名（兼容连字符/下划线差异），避免市场中的同名插件被张冠李戴。
- **Bug 修复**：
  - 修复获取市场数据失败时会把整个本地插件列表当作"待更新列表"返回的问题（现在会安全地跳过本次检查）。
  - 修复更新结果摘要不显示版本变化的问题，现在会显示 `插件名 (旧版本 -> 新版本)`。
  - 通过 `StarMetadata.reserved` 标记跳过系统保留插件，不再依赖名称猜测。
