...
- 新增管理员指令 `预览插件更新汇报`（`previewpluginupdate`），在当前会话按当前 `update_report_fields` 配置展示模拟更新检查通知。
- 新增 `更新管理帮助`（`updatemanagerhelp`），按插件更新、插件维护和 AstrBot 框架分类返回全部管理员指令。
- `测试插件更新日志` 更名为 `测试插件日志`（保留原命令兼容），使用本机已加载插件、其最新 CHANGELOG 小节和当前汇报配置；框架更新开启时附加最近一次 AstrBot 发布日志。
